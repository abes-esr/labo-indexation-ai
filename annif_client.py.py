...
if __name__ == "__main__":
    ...
    for project in annif.projects:
        print(
            "Project id: {:<16} lang: {}  name: {}".format(
    ...
    results = annif.suggest(
        project_id="yso-en", text="The quick brown fox jumped over the lazy dog"
    )
    for result in results:
        print(
            "<{}>\t{:.4f}\t{}".format(result["uri"], result["score"], result["label"])
        )
    ...
    with open("LICENSE.txt") as license_file:
        results = annif.suggest(project_id="yso-en", text=license_file, limit=5)
        for result in results:
            print(
                "<{}>\t{:.4f}\t{}".format(
